...
There is thus good reason to be wary of the use of three dimensions where only two are necessary to convey data [@vanderplasTestingStatisticalCharts2020;@wilkeDonGo3D2020;@morganwallIntroducing3DGgplots20191006]. 
...
```{r midmeans-log-errors, fig.cap='Midmeans and observed values of log absolute errors for the true ratio of bars. Summary lines are computed from raw data using a loess smooth.', message=FALSE, warning=FALSE, out.width='80%'}
...
mod <- lmer(response ~ (1|subject) + factor(ratio)+plot+factor(type),
            data = res)
modsum <- summary(mod)
modaov <- anova(mod)
