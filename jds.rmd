...
As a result, it is reasonable to reconsider the use of 3D charts, not only because of new technological developments, but also because 3D printed charts provide the opportunity to make data graphics accessible to those with limited or absent vision [@fleetHowTactileGraphics2023].
...
\svp{In each graph, there are two sets of five bars each, and two of the bars are marked for identification with a circle and triangle}, \svp{as in \Cref{fig:bar-types}.}
Cleveland and McGill did not specify the random process for the heights of the eight other bars, so we used a scaled Beta distribution with parameters that limit excessive noise around the bars used for comparisons.\svp{What does excessive noise mean??}
...
```{r demographics, fig.cap="Demographic characteristics of participants in the study.", message=FALSE, warning=FALSE, out.width='100%', fig.width = 9, fig.height = 3.5}
library(patchwork)
...
All responses ($n = `r nrow(res_all) - nrow(res)`$) that incorrectly identified the smaller bar were removed from the study before analysis; this serves as a basic attention check.
...
```{r midmeans-log-errors, fig.cap='Midmeans and observed values of log absolute errors for the true ratio of bars. Summary lines are computed from raw data using a loess smooth.', message=FALSE, warning=FALSE, out.width='\\textwidth'}
...
<!-- No differences were detected for the true ratio of bars ($t = `r modsum$coefficients[2,3]`$), whether the bars were adjacent or separated ($t = `r modsum$coefficients[5,3]`$), or for the plot nested within the true ratio ($F(`r modaov$npar[2]`, `r f_df`) = `r modaov[2,4]`$).  -->
<!-- The form of this linear mixed effects model assumes a linear relationship between ratio and response, but we know from \Cref{fig:midmeans-log-errors} that the actual relationship appears to be nonlinear.  -->
...
gam_mod <- gam(response ~ s(ratio_prop, by = plot, k = 6) + s(ratio_prop, by = type, k = 6) +  plot + type + s(participant, bs = "re"),
...
res %>%
  ...
  xlab("# Clicks on WebGL Plot") + ylab("Count") + ggtitle("Accuracy and Participant Interactivity") + 
...
The first potential explanation is that this study is underpowered - the effect size is small, and our 48 participants were insufficient; the original study included 51 participants, which is slightly larger. However, we should note that the analysis methods used in @heerCrowdsourcingGraphicalPerception2010b and @clevelandGraphical1984 do not provide numerical tests of statistical significance, instead defaulting to graphical displays which include confidence intervals.
