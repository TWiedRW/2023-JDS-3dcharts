...
A total of `r nrow(valid.users)` participants completed the study; demographics are shown in \Cref{fig:demographics}.
The participants provided a total of `nrow(res_all)` responses.
...
All responses that incorrectly identified the smaller bar ($n = `r nrow(res_all) - nrow(res)`$) were removed from the study before analysis; this serves as a basic attention check and leaves `r nrow(res)` valid responses. In addition, one response was removed because the participant did not enter a valid ID for a 3D printed chart.
