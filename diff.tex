...
%DIF DEL initialsubmission.tex   Sun Jan 21 19:15:37 2024
%DIF ADD jds.tex                 Mon Jan 22 00:30:32 2024
...
All responses \DIFaddbegin \DIFadd{(\(n = 32\)) }\DIFaddend that incorrectly identified the smaller bar were removed from the study before analysis\DIFaddbegin \DIFadd{; this serves as a basic attention check}\DIFaddend .
...
The results \DIFdelbegin \DIFdel{tend to indicate }\DIFdelend \DIFaddbegin \DIFadd{suggest }\DIFaddend that the log absolute errors increase for greater differences between the smaller and larger bars, but are consistent across the graph types. \DIFaddbegin \DIFadd{In addition, there is a tendency
}\DIFaddend 
...
In addition to replicating the (primarily graphical) analysis of participant errors, we also took a more statistical approach and fitted a \DIFdelbegin \DIFdel{linear mixed effects }\DIFdelend \DIFaddbegin \DIFadd{generalized additive }\DIFaddend model that accounts for participant variation as well as the effect of comparison type, graph type, and ratio. \DIFdelbegin \DIFdel{This allows us to test for significant differences (though given the midmean log absolute error plots, we do not expect to find any) as well as to quantify effect sizes for future studies. }\DIFdelend The formal statistical model is as follows:

\[y_{ijklm}= \DIFdelbegin \DIFdel{\mu+S_i+}\DIFdelend \DIFaddbegin \DIFadd{s(}\DIFaddend R_j \DIFaddbegin \DIFadd{* T_l) }\DIFaddend + \DIFdelbegin \DIFdel{G}\DIFdelend \DIFaddbegin \DIFadd{s}\DIFaddend (R\DIFaddbegin \DIFadd{_j *G_k}\DIFaddend ) \DIFdelbegin \DIFdel{_{(k)j}}\DIFdelend + T_l + \DIFaddbegin \DIFadd{G_k + s(S_i) + }\DIFaddend \epsilon_{ijklm}\]
...
\item
  \DIFdelbegin \DIFdel{\(S_i\sim N(0,\sigma^2_S)\) is the }\DIFdelend \DIFaddbegin \DIFadd{\(s(S_i)\) is the random }\DIFaddend effect of the \(i^{th}\) subject
\item
  \DIFdelbegin \DIFdel{\(R_j\) }\DIFdelend \DIFaddbegin \DIFadd{\(s(R_j*T_l)\) }\DIFaddend is the effect of the \(j^{th}\) ratio\DIFaddbegin \DIFadd{, by comparison type
}\DIFaddend \item
  \DIFdelbegin \DIFdel{\(G(R)_{(k)j}\) }\DIFdelend \DIFaddbegin \DIFadd{\(s(R_j*G_k)\) }\DIFaddend is the effect of the \DIFdelbegin \DIFdel{\(k^{th}\) graph type nested in the }\DIFdelend \(j^{th}\) ratio\DIFaddbegin \DIFadd{, by display type
}\DIFaddend \item
  \(T_l\) is the effect of the \(l^{th}\) comparison type
\item
  \DIFaddbegin \DIFadd{\(G_k\) is the effect of the \(k^{th}\) display type
}\item
  \DIFaddend \(\epsilon_{ijklm}\sim N(0,\sigma^2_\epsilon)\) is the random error
\end{itemize}

\DIFdelbegin \DIFdel{No differences were detected for the true ratio of bars (p-value = .7881) }\DIFdelend \DIFaddbegin \DIFadd{A generalized additive model with random effects for subject was fit using the }\texttt{\DIFadd{mgcv}} \DIFadd{package \mbox{%DIFAUXCMD
...
\begin{tabular}[t]{lrrrr}
\toprule
\DIFaddFL{Smooth }& \DIFaddFL{edf }& \DIFaddFL{Ref.df }& \DIFaddFL{F }& \DIFaddendFL p-value\DIFdelbeginFL \DIFdelFL{= .6868).
While Heer \& Bostock }\DIFdelendFL \DIFaddbeginFL \\
\midrule
...
\addlinespace
\DIFaddFL{s(participant) }& \DIFaddFL{27.0449 }& \DIFaddFL{34.0000 }& \DIFaddFL{4.7264 }& \DIFaddFL{0.0000}\\
\bottomrule
\end{tabular}
\end{table}
...
Plots of the residual diagnostics can be found under the supplemental material in \mbox{%DIFAUXCMD
\Cref{fig:residualplots}}\hskip0pt%DIFAUXCMD
.
}
